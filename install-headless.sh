...
DATABASE_URL="file:$INSTALL_DIR/prisma/dev.db"
NODE_ENV="production"
PORT=3000
HOST="0.0.0.0"
HA_ROLE="PRIMARY"
...
# Vorab-Rettungsschritt für Float->Cent, falls altes Schema vorhanden
if [ -f "$INSTALL_DIR/scripts/migrate-v0417-cent.js" ]; then
  sudo -u "$SERVICE_USER" DATABASE_URL="file:$INSTALL_DIR/prisma/dev.db" node "$INSTALL_DIR/scripts/migrate-v0417-cent.js" 2>/dev/null || true
fi

sudo -u "$SERVICE_USER" DATABASE_URL="file:$INSTALL_DIR/prisma/dev.db" npx prisma generate
...
DBPUSH_RC=0
DBPUSH_OUTPUT=$(sudo -u "$SERVICE_USER" DATABASE_URL="file:$INSTALL_DIR/prisma/dev.db" npx prisma db push --skip-generate 2>&1) || DBPUSH_RC=$?
...
if [ "$DBPUSH_RC" -eq 0 ]; then
  ...
  if [ "${OPENBON_ALLOW_DATA_LOSS:-}" = "1" ]; then
    ...
    sudo -u "$SERVICE_USER" DATABASE_URL="file:$INSTALL_DIR/prisma/dev.db" npx prisma db push --accept-data-loss --skip-generate
  else
...
if [ "$IS_UPDATE" -eq 0 ]; then
  sudo -u "$SERVICE_USER" DATABASE_URL="file:$INSTALL_DIR/prisma/dev.db" node prisma/seed.js 2>/dev/null || true
fi
...
sudo -u "$SERVICE_USER" DATABASE_URL="file:$INSTALL_DIR/prisma/dev.db" npm run build
...
Environment=DATABASE_URL="file:$INSTALL_DIR/prisma/dev.db"
